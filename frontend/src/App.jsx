...
import RoleSelector, { DEFAULT_ROLE, getStoredRole, storeRole } from './components/RoleSelector'
...
export default function App() {
  ...
  // Restore the persisted role once on mount.
  useEffect(() => {
  ...
  const locationBar = (
    <section aria-label={t(language, 'locationLabel')} className="bg-gradient-to-b from-navy-900 to-navy-800 border border-border rounded-2xl p-4 flex flex-col gap-3">
      <form onSubmit={searchPlace} className="flex flex-col sm:flex-row gap-2 sm:items-center" role="search">
        <label className="flex flex-col gap-1 text-[11px] uppercase tracking-wide text-slate-300 font-semibold flex-1">
  ...
  return (
    ...
    <div className="min-h-screen">
      ...
      <div className="max-w-[1440px] mx-auto p-5 flex flex-col gap-4">
        <RoleSelector role={role} onChange={handleRoleChange} language={language} />
        {locationBar}
